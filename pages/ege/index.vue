        ...
        <div class="min-h-screen  relative overflow-hidden">
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 py-8 z-10">
                ...
                <div class="flex gap-4 mb-8">
                    <button v-for="level in levels" :key="level.id" @click="selectedLevel = level"
                        class="px-6 py-3 rounded-xl font-medium transition-all duration-300"
                        :class="selectedLevel?.id === level.id ? 'bg-gradient-to-r from-neutral-400 to-neutral-900 text-white shadow-lg' : 'bg-neutral-800/50 text-neutral-400 hover:text-white'">
                        {{ level.name }}
                    </button>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div v-for="subject in subjects" :key="subject.id" @click="goToSubject(subject.id)"
                        class="group relative bg-neutral-900/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 cursor-pointer hover:border-neutral-300 transition-all duration-300 hover:scale-105 hover:shadow-xl">
                        <!-- Кнопки админа -->
                        <div v-if="isAdmin"
                            class="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition"
                            @click.stop>
                            <button @click="editSubject(subject)"
                                class="p-1 bg-neutral-800 rounded-full hover:bg-neutral-700">
                        ...
                        <div class="mt-4 text-neutral-200 text-sm opacity-0 group-hover:opacity-100 transition">Перейти →
                        </div>
                    </div>
                </div>

                <!-- Кнопка добавления предмета (только для админа) -->
                <button v-if="isAdmin" @click="openAddSubjectModal"
                    class="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-r from-neutral-500 to-neutral-800 rounded-full flex items-center justify-center text-white text-3xl shadow-lg hover:scale-110 transition">
                    +
                </button>
            </div>
            ...
            <Teleport to="body">
                <div v-if="showSubjectModal"
                    ...
                    <div class="bg-neutral-900 rounded-2xl max-w-md w-full p-6 border border-white/10">
                        <h2 class="text-xl font-bold text-white mb-4">{{ editingSubject ? 'Редактировать предмет' :
                            'Добавить предмет' }}</h2>
                        <input v-model="subjectForm.name" placeholder="Название предмета"
                            class="w-full p-3 bg-neutral-800 border border-white/10 rounded-xl text-white mb-4" />
                        <div class="flex justify-end gap-2">
                            <button @click="closeSubjectModal"
                                class="px-4 py-2 bg-neutral-800 rounded-lg text-neutral-300">Отмена</button>
                            <button @click="saveSubject"
                                class="px-4 py-2 bg-neutral-600 hover:bg-neutral-500 rounded-lg text-white transition-all duration-400">Сохранить</button>
                        </div>
                    </div>
                </div>
            </Teleport>
