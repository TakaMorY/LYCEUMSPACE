...
const originalAvatar = ref('') // для возможного сравнения
...
// Дебаунс проверки уникальности никнейма
...
const handleAvatarUpload = async (event) => {
  if (!user.value) {
    avatarError.value = 'Необходимо войти в систему'
    return
  }
  ...
  try {
    ...
    // 3. Сохраняем URL в таблицу profiles (!!! самое важное !!!)
    ...
    // 5. Инвалидируем кэш, чтобы новый аватар появился на других страницах
    refreshNuxtData('feed')
    // Также можно сбросить кэш текущей страницы, если нужно
    // refreshNuxtData('profile-settings')

    // Очищаем input, чтобы можно было загрузить тот же файл снова
    event.target.value = ''
  } catch (err) {
    console.error('Ошибка при загрузке аватара:', err)
    avatarError.value = 'Ошибка при загрузке. Убедитесь, что bucket "avatars" существует и доступен.'
  } finally {
