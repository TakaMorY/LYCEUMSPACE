<template>
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center space-x-4 border-b border-neutral-800 pb-6 mb-6">
            <img :src="avatarPreview || '/images/defaultavatar/default-avatar.png'"
        ...
        <div class="space-y-6">
            ...
            <div class="flex justify-end space-x-3 pt-4">
                ...
                <button @click="saveProfile" :disabled="savingProfile || uploadingAvatar"
...
// Получаем доступ к глобальному состоянию профиля из layout
const headerProfile = useState('header-profile')
...
const handleAvatarUpload = async (event) => {
    ...
    try {
        ...
        // Обновляем глобальное состояние для шапки
        if (headerProfile.value) {
            headerProfile.value.avatar_url = publicUrl
        } else {
            // Если по какой-то причине профиль ещё не загружен, принудительно перезагрузим
            refreshNuxtData('header-profile')
        }
...
const saveUsername = async () => {
    ...
    try {
        ...
        // Обновляем глобальное состояние для шапки
        if (headerProfile.value) {
            headerProfile.value.username = form.value.username
        }
