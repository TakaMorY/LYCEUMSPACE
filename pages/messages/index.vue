      ...
      <div class="w-full md:w-80 border-b md:border-b-0 md:border-r border-neutral-800 flex flex-col overflow-hidden">
        ...
        <div v-if="searchQuery" class="flex-1 overflow-y-auto">
          <div v-if="searching" class="text-center py-8">
            <div class="inline-block w-6 h-6 border-2 border-neutral-500 border-t-transparent rounded-full animate-spin"></div>
          </div>
          ...
          <div v-else>
            <button v-for="user in userResults" :key="user.id" @click="startChat(user)"
              ...
              <img :src="user.avatar_url || '/images/defaultavatar/default-avatar.png'" class="w-10 h-10 rounded-full" />
              <div>
        ...
        <div v-else class="flex-1 overflow-y-auto">
          <!-- Избранное -->
          <button @click="openFavorites" class="w-full p-3 text-left hover:bg-neutral-800/50 transition-colors flex items-center gap-3">
          ...
          <button v-for="chat in chats" :key="chat.user.id" @click="selectChat(chat.user)"
            ...
            <img :src="chat.user.avatar_url || '/images/defaultavatar/default-avatar.png'" class="w-10 h-10 rounded-full" />
            <div class="flex-1 min-w-0">
              <div class="font-medium text-white truncate">{{ chat.user.full_name || chat.user.username }}</div>
              <div class="text-sm text-neutral-400 truncate">@{{ chat.user.username }}</div>
            </div>
            <div v-if="chat.unread > 0" class="w-5 h-5 bg-red-500 rounded-full text-xs flex items-center justify-center text-white">
        ...
        <div v-if="selectedUser" class="flex-1 flex flex-col">
          ...
          <div class="p-4 border-b border-neutral-800 flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div v-if="isFavorites" class="w-10 h-10 rounded-full bg-pink-500/20 flex items-center justify-center">
                <svg class="w-5 h-5 text-pink-400" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                </svg>
              </div>
              <img v-else :src="selectedUser.avatar_url || '/images/defaultavatar/default-avatar.png'" class="w-10 h-10 rounded-full" />
              <div>
            ...
            <NuxtLink v-if="!isFavorites" :to="`/profile/${selectedUser.id}`" class="text-sm text-blue-400 hover:underline">
              Профиль
            </NuxtLink>
          </div>
          ...
          <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-3">
            <div v-for="msg in messages" :key="msg.id" class="flex" :class="msg.sender_id === currentUserId ? 'justify-end' : 'justify-start'">
              <div :class="msg.sender_id === currentUserId ? 'bg-blue-600' : 'bg-neutral-800'" class="max-w-[75%] rounded-2xl p-3 shadow-sm">
                ...
                <img v-if="msg.image_url" :src="msg.image_url" class="mt-2 max-h-48 rounded-lg cursor-pointer" @click="openImage(msg.image_url)" />
                <div class="flex items-center justify-end gap-1 mt-1">
                  <span class="text-[10px] text-neutral-400">{{ formatTime(msg.created_at) }}</span>
                  <span v-if="msg.sender_id === currentUserId && msg.read" class="text-[10px] text-blue-300">✓✓</span>
                  <span v-else-if="msg.sender_id === currentUserId" class="text-[10px] text-neutral-400">✓</span>
                </div>
              </div>
            </div>
            <div v-if="loadingMessages" class="text-center py-4">
              <div class="inline-block w-5 h-5 border-2 border-neutral-500 border-t-transparent rounded-full animate-spin"></div>
            </div>
          </div>
          ...
          <div class="p-4 border-t border-neutral-800">
            <div class="flex flex-col gap-2">
              <div v-if="imagePreview" class="relative inline-block self-start">
                ...
                <button @click="clearImage" class="absolute -top-2 -right-2 bg-red-500 rounded-full p-1">
              ...
              <div class="flex items-end gap-2">
                <label class="cursor-pointer p-2 bg-neutral-800 hover:bg-neutral-700 rounded-full transition">
                ...
                <textarea
                  ...
                  rows="1"
                  class="flex-1 px-4 py-2 bg-neutral-800 rounded-xl text-white placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-blue-600 resize-none"
                ></textarea>
                <button @click="sendMessage" :disabled="(!newMessageText.trim() && !imageFile) || sending"
              ...
              <div v-if="uploadingImage" class="text-xs text-neutral-400 flex items-center gap-2">
                <div class="inline-block w-3 h-3 border-2 border-neutral-500 border-t-transparent rounded-full animate-spin"></div>
                Загрузка фото...
              </div>
            </div>
          </div>
  ...
  try {
    ...
    const chatsWithUnread = await Promise.all(profiles.map(async (profile) => {
      const { count } = await supabase
        ...
        .eq('read', false)
      return { user: profile, unread: count || 0 }
    }))
  ...
  messagesChannel = supabase
    ...
    .on('postgres_changes', {
      ...
      if (selectedUser.value) {
        if (isFavorites.value && payload.new.sender_id === currentUserId.value && payload.new.receiver_id === currentUserId.value) {
          if (!messages.value.some(m => m.id === payload.new.id)) {
            messages.value.push(payload.new)
            await scrollToBottom()
          }
        } else if (!isFavorites.value && payload.new.sender_id === selectedUser.value.id) {
          if (!messages.value.some(m => m.id === payload.new.id)) {
            messages.value.push(payload.new)
            await scrollToBottom()
            await supabase
