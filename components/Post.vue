<template>
  <article
    class="relative group bg-neutral-900/40 backdrop-blur-xl rounded-2xl sm:rounded-3xl border border-neutral-700/50 overflow-hidden hover:shadow-2xl transition-all duration-300">
    <div class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none">
      <div class="absolute top-0 left-0 w-20 h-20 bg-gradient-to-br from-white/20 to-transparent rounded-full blur-2xl">
      </div>
      <div
    ...
    <div v-if="isAuthor" class="absolute top-4 right-4 z-10">
      <button @click="deletePost" :disabled="isDeleting"
        class="p-2 bg-red-500/20 hover:bg-red-500/40 rounded-lg transition-all duration-300 backdrop-blur-sm"
        title="Удалить пост">
    ...
    <!-- Изображение поста -->
    <div v-if="post.image_url" class="w-full max-h-[60vh] sm:max-h-[70vh] overflow-hidden cursor-pointer"
    ...
    <div class="relative p-4 sm:p-6">
      ...
      <div class="flex items-start gap-2 sm:gap-3 mb-2 sm:mb-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <NuxtLink :to="`/post/${post.id}`" class="block hover:underline">
      ...
      <!-- Текст поста -->
      <p class="text-white/80 text-sm sm:text-base mb-3 sm:mb-4 leading-relaxed whitespace-pre-wrap">{{ post.content }}
      </p>

      <!-- Действия -->
      <div class="flex items-center gap-4 sm:gap-6">
        <button @click="toggleLike" :disabled="isProcessing" class="flex items-center gap-1 sm:gap-2 group">
          <svg class="w-5 h-5 sm:w-6 sm:h-6 transition-all duration-300" :class="[
          ...
          <span class="text-white text-sm sm:text-base font-medium">{{ likesCount }}</span>
        </button>

        <button @click="toggleComments" class="flex items-center gap-1 sm:gap-2 group">
          <svg class="w-5 h-5 sm:w-6 sm:h-6 text-neutral-400 group-hover:text-blue-400 transition-colors" fill="none"
      ...
      <div v-if="showComments" class="mt-4 sm:mt-6 space-y-3 sm:space-y-4">
        <Comment v-for="c in comments" :key="c.id" :comment="c" @open-image="openImage" />

        <!-- Форма добавления комментария -->
        <div v-if="user"
          class="mt-4 bg-neutral-800/40 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-neutral-700/30">
          <textarea v-model="newCommentText" placeholder="Напишите комментарий..." rows="2"
            class="w-full bg-transparent border-b border-neutral-700 text-white placeholder-neutral-600 focus:border-white focus:outline-none transition-all duration-300 resize-none text-sm sm:text-base"></textarea>
          <div class="flex flex-wrap items-center gap-3 mt-3">
            <label
              class="cursor-pointer px-3 py-2 sm:px-4 sm:py-2 bg-neutral-800 hover:bg-neutral-700 rounded-lg text-xs sm:text-sm text-white transition border border-neutral-700/50 flex items-center gap-2">
            ...
            <button @click="addComment" :disabled="!newCommentText.trim() || commentUploading"
...
const isAuthor = computed(() => user.value?.id === props.post.user_id)
...
watch(() => props.post.user_liked, (newVal) => {
  if (!isProcessing.value) userLiked.value = newVal
})
watch(() => props.post.likes_count, (newVal) => {
  if (!isProcessing.value) likesCount.value = newVal
})
...
const setupCommentsRealtime = () => {
  ...
  commentsChannel = supabase
    ...
    .on('postgres_changes', {
      ...
      // Обновляем счётчик комментариев в посте (для родителя)
      emit('updated', {
        id: props.post.id,
...
const toggleLike = async () => {
  ...
  try {
    ...
    const oldLiked = userLiked.value
    const oldCount = likesCount.value
    ...
    if (newLiked) {
      ...
      if (error) {
        if (error.code === '23505') {
          userLiked.value = oldLiked
          likesCount.value = oldCount
          return
        }
        throw error
      }
    ...
    if (e.code !== '23505') alert('Не удалось поставить лайк')
  } finally {
...
const deletePost = async () => {
  ...
  try {
    if (props.post.image_url) {
      const fileName = props.post.image_url.split('/').pop()
